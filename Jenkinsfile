node ("Dockerhost") {
    ...
    stage ('Deploy') {
        ...
        sh "docker run --rm -d -p 8088:8088 --name ${JOB_NAME}-${BUILD_NUMBER} -v \$(pwd)/target:/app -e JAVA_APP_JAR=/app/${pom.artifactId}-${pom.version}.jar fabric8/java-alpine-openjdk8-jdk"
    }
}
